package chap07.practice;

public class Test4 {

    public static void main(String[] args) {
//    	전기세를 계산하는 프로그램을 작성하세요.
    	
    	/*
    	ex)
			전력 사용량은 201 ~ 400kwh 이하구간이다.
			기본요금 : 1,600
			1단계 : 200kwh * 93.3원(1~200kwh까지는 93.3원이다.)
			2단계 : 200kwh * 187.9원(201~400kwh까지는 187.9원이다.)
			사용량은 키보드로부터 입력 받아서 계산한다.
			- 소수점 이하 절사방법(Math.floor(n)으로 절사 ex)Math.floor(93.3);)
			- 소수점 이하 반올림방법(Math.round(n)으로 반올림 ex)Math.round(93.3);)

			사용량 201kwh는 1,600 + 18,847원 = 20,447원
			부가가치세(원미만 4사5입) : 20,447원 * 0.1 = 2,045원
			전력산업기반기금(10원미만 절사) : 20,447원 * 0.037 = 750원
			청구금액(전기요금계 + 부가가치세 + 전력산업기반기금)
			: 20,447원 + 2,045원 + 750원 = 23,240원(10원미만 절사)
			
			청구금액 : 23240
		*/
    }

}
